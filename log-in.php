...
//echo to user that there is an error for security reason the error message will be general
    ...
    //search from sponsor table in database for entered email 
       $query = "SELECT * FROM `sponsor` WHERE `email_address`='$email_address'";
        $results = mysqli_query($connection, $query);
if (mysqli_num_rows($results) > 0) {
  // output data of each row
  while($row = mysqli_fetch_assoc($results)) {
 ...
 $name=$row['name'];
  ...
  $bool=hash_equals($hashed_password,$SavedPass );
  if($bool==1){
       //Create sesstion and redirect sponsor to home.php
       //save attribute is session array to use it later in authentication
    ...
    <label for="radioSponsor">راعي</label> 
               ...
               <div class="col-lg-6">
                <div class="card1 pb-5">
                    <div class="row px-3 justify-content-center mt-4 mb-5 border-line"> <img src="layout/logIn.png" class="image"> </div>
                </div>
            </div>
                 </form>
        </div>
