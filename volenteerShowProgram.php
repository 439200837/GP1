...
<element dir="rtl">
      ...
      '<td class="column1">اسم البرنامج </td>'
           .'<td class="column2">الحالة</td>'.'<td class="column3">ملاحظة</td>'.' </tr>  
            </thead>';
            
       //retrieving the enrollment information of the volunteer from database
                    $sql="SELECT * FROM `enroll`";
                    $sql2 = "SELECT program.name, volunteer.id, enroll.program_id, enroll.id, enroll.note, enroll.status FROM program, volunteer, enroll WHERE volunteer.id = enroll.volenteer_id AND enroll.program_id = program.id AND enroll.volenteer_id=$id";
