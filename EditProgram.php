<?php
session_start();
//check if user is admin or member or else
//for non member the page will not be athorize to access
...
foreach($checkbox1 as $chk1)  
   {  
    ...
    if($image==""){
       $sql_update = "UPDATE `program` SET `name`='$proname', `start_date`='$start_date', `end_date`='$end_date', `type`='$chk', `description`='$prodes' WHERE `id`='$id2'";  
         echo "<br>";
       }    
   ...
   elseif($image!="") { 
        // Get file info 
       $image = addslashes(file_get_contents($_FILES['images']['tmp_name']));
        $sql_update = "UPDATE `program` SET `name`='$proname', `start_date`='$start_date', `end_date`='$end_date', `type`='$chk', `description`='$prodes', `picture`='$image' WHERE `id`='$id2'";
       }
